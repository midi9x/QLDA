...
<?php
	$id_nhom=$_GET["idn"];
	$sql="select * from nhomhang where id_nhom=$id_nhom";
	$kq=mysql_query($sql);
...
<?php	
	$sql2="select * from loaihang where id_nhom=$id_nhom";
	$kq2=mysql_query($sql2);
	while($r2=mysql_fetch_array($kq2))
	{
		$id_loai=$r2["id_loai"];$tenloaisp=$r2["tenloaisp"];
		$query="select count(*) from hang where id_loai=$id_loai";			
...
?>	
	...
	<article class="wrap-home-category">
		<header class="title">
			<h3  class="title bold-font"><b> <a style="color:#fff;" href="/loai-san-pham/<?=khongdau($tenloaisp)?>-<?php echo $id_loai; ?>.html"><?php echo "$tenloaisp"; ?></a>   </b></h3> 
		</header>
