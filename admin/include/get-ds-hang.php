...
if(isset($_POST["act"]))
{
	...
	{	
			echo "<script>var check=window.confirm('Bạn chắc chắn muốn xóa Hàng này');
			if(check==true) ";

			$sql3="Delete from hang where hang in $ma";
			...
			else
			{
				...
				echo "<script>alert('Đã xóa $n Hàng!');window.location='../admin/?page=ds-hang';</script>";
			}			
...
<?php
	...
	//******************************** Nội Dung *********************************//
		$sql3="select * from hang order by tensp limit $k,$pagesize";
		$kq3=mysql_query($sql3);
		if(!$kq3)
			echo "";
		else{
		while($r3=mysql_fetch_array($kq3))
		{
			...
			$sql2="select * from loaihang,hang where loaihang.id_loai=hang.id_loai and hang.id='$id'";
  ...
  <td width="50" align="center" style="border-left:1px solid #333;border-bottom:1px solid #333; border-right:1px solid #333"><input type="checkbox" name="chon[]" value="<?php echo $id; ?>"/></td>  
    ...
    <td width="100" align="center" style="border-bottom:1px solid #333; border-right:1px solid #333"><img src="../hinhanh/thumb/<?php echo "$hinh"; ?>" width="90" height="90"></td>
    ...
    <td width="50" align="center" style="border-bottom:1px solid #333; border-right:1px solid #333"><a href="?page=cap-nhat-hang&id=<?php echo $id; ?>">Sửa</a></td>
    <td width="50" align="center" style="border-bottom:1px solid #333; border-right:1px solid #333"><a href="?page=xoa-hang&id=<?php echo $id; ?>" onclick="return check()">Xóa</a>
    </td>
  </tr>
 <?php
	  		}
 ...
 <?php
    ...
    if($curseg>1)
        echo "<a href='?page=ds-hang&page=".(($curseg-1)*$segsize)."'><b>Previous</b></a> &nbsp;";
        $n=$curseg*$segsize<=$pagecount?$curseg*$segsize:$pagecount;
        for($i=($curseg-1)*$segsize+1;$i<=$n;$i++)
		{
			if($curpage==$i)
				echo "<a href='?page=ds-hang&page=".$i."'><font color='#FF0000'>[".$i."]</font></a> &nbsp;";
			else
				echo "<a href='?page=ds-hang&page=".$i."'><font color='#FFF'>[".$i."]</font></a> &nbsp;";
		}
        if($curseg<$numseg)
		echo "<a href='?page=ds-hang&page=".(($curseg*$segsize)+1)."'><b>Next</b></a> &nbsp;";		
	}
?>
    </td>
  </tr>
