<?php
	...
	if($idloai=="chonlsp"){
		include "get-ds-hang.php";
	}
	else{	
        $kq=mysql_query("select count(*) from hang where id_loai=$idloai"); 
	...
	//******************************** Nội Dung *********************************//
		$sql3="select * from hang where id_loai=$idloai order by id_loai limit $k,$pagesize";
		...
		while($r3=mysql_fetch_array($kq3))
		{
			...
			while($r2=mysql_fetch_array($kq2))
			{
				...
				<td width=100 align=center style=\"border-bottom:1px solid #333; border-right:1px solid #333\"><img src=\"../hinhanh/thumb/$hinh\" width=90 height=90></td>
      ...
      $s.="<tr>
        ...
        if($curseg>1)
            $s.="<a href='?page=ds-hang&page=".(($curseg-1)*$segsize)."'><b>Previous</b></a> &nbsp;";
            $n=$curseg*$segsize<=$pagecount?$curseg*$segsize:$pagecount;
            for($i=($curseg-1)*$segsize+1;$i<=$n;$i++)
            {
                if($curpage==$i)
                   $s.="<a href='?page=ds-hang&page=".$i."'><font color='#FF0000'>[".$i."]</font></a> &nbsp;";
                else
                    $s.= "<a href='?page=ds-hang&page=".$i."'><font color='#FFF'>[".$i."]</font></a> &nbsp;";
            }
            if($curseg<$numseg)
            $s.="<a href='?page=ds-hang&page=".(($curseg*$segsize)+1)."'><b>Next</b></a> &nbsp;";		
        }
